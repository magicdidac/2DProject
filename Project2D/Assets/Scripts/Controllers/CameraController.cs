...
public class CameraController : AController
{
    ...
    void FixedUpdate()
    {
        ...
        {
            if (gc.player.transform.position.y < myFloor * 8 + 3)
            {
            ...
            {
                transform.position = Vector3.Lerp(transform.position, new Vector3(gc.player.transform.position.x + _offset, gc.player.transform.position.y - 1, transform.position.z), .5f);
